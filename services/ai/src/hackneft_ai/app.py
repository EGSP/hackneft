...
3. При необходимости добавить карточку агента: `POST /api/agents`. Карточка задаёт
   системный промпт, закреплённые инструкции из `/api/instructions` и модель сессии.
   При первом запуске новой версии штатные карточки обновляются до шести причин
   и двух режимов с архивом прежних текстов.
